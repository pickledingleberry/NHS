...
def search_autozone(
    ...
    try:
        with browser_page(engine="firefox") as page:
            ...
            # Click Continue (first step submit)
            page.locator('button[type="submit"]').first.click(force=True)
            page.wait_for_timeout(3000)

            # --- Step 2: Enter password on second screen ---
            ...
            # Ensure "Enter my password" option is selected
            for selector in [
                'input[type="radio"][value*="password" i]',
                'label:has-text("Enter my password")',
            ]:
                try:
            ...
            cards = page.locator('[data-testid*="product"], [class*="product"], [class*="part"], article, li')
            parsed: list[dict] = []
